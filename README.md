...
бэк будет доступен по localhost:8081/weather/api/ (эндпоинты постмана для проверки так же в папке postman)

Рекомендую проходить по всем эндпоинтам в указанном порядке
...
- небольшой rest сервис для работы автозаполнения и загрузки изображений на imgbb.com(исходники можно посмотреть тут https://github.com/MrShoffen/cities_api )
...
```
```
!!! avatarUrl - url аватара. можно использовать любой url длиной до 256 символов.
В моем фронтенде можно загружать аватар с устройства - 
для этого используется небольшое spring boot приложение с эндпоинтом для загрузки картинки.


В качестве хостинга для сохранения картинок используется https://imgbb.com/
```
